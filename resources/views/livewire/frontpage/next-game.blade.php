@if ($next_match)
    <div class="flex flex-col w-full">
        <div class="flex items-center">
            ...
            <!-- away -->
            <div class="flex-1 flex-col text-center font-bold">
                <img src="{{ $next_match->match->teamAway->logo() }}" class="m-auto w-auto"/>
                <span>{{ $next_match->match->teamAway->name_short  }}</span>
            </div>
        </div>
        <div class="text-center">
            <i class="fas fa-map-marker-alt text-red-500"></i> {{ $next_match->location ? $next_match->location->name : "-" }}
        </div>

        {{-- participate --}}
        @auth
            @if ($next_match->clubs->intersect(auth()->user()->player->clubs)->isNotEmpty() && $next_match->poll_begins && $next_match->poll_ends)
