...
class Poll extends Component
{
    ...
    public function mount(Date $date)
    {
        ...
        foreach ($this->date->clubs as $club)
        {
            ...
            {
                $this->date_players = $club->players()->whereHas('user')->get()->where('pivot.playerStatus.display_in_polls');
            } else {
                $this->date_players = $this->date_players->merge($club->players()->whereHas('user')->get()->where('pivot.playerStatus.display_in_polls'));
            }
        }
    }
